...
static const std::unordered_map<std::string_view, Entry> kTable {
    ...
    { "ComputeGraph",       { CompatClass::NNSpireExtension, "" } },
    { "ActivationsFnLayer", { CompatClass::NNSpireExtension, "" } },
};
...
CompatClass CompatibilityChecker::classify(std::string_view typeName) noexcept
{
    ...
    if (it == kTable.end()) {
        return CompatClass::NNSpireExtension;
    }
...
std::string CompatibilityChecker::onnxOp(std::string_view typeName)
{
    ...
    if (it == kTable.end()) {
        ...
        return std::string("com.NNSpire.") + std::string(typeName);
    }
