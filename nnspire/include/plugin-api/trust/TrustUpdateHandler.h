 ...
 * WHAT IS A TUP?
 * ───────────────
 * A Trust Update Packet (.tup) is a signed bundle that instructs NNSpire
 ...
 * cannot issue them (they can only request them via NNSpire-sign submit).
 *
 * TUP FILE FORMAT (.tup)
 * ──────────────────────
 *   [4]  magic     "NNTU"
 ...
 *   if (!r.ok()) { log(r.error()); }
 *   store.save("/etc/NNSpire/trust.nnts");   // persist the update
 */
