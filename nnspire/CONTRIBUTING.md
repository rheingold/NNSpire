# Contributing to NNSpire — Repository Architecture
...
> For what NNSpire does as software, read [README.md](README.md) (once created).
...
3. `include/builtin/` + `src/builtin/` — NNSpire's own implementations: Dense, CpuBackend, Adam, MSE
...
6. `python-bridge/` — pybind11 bridge wrapping `NNSpire-core` for Python (Phase 2)
...
|---|---|---|
| 1 — public contract | `NNSpire::core::` | Interfaces (`ILayer`, `IBackend`, …), data types (`Tensor`, `Result<T>`) |
| 2 — engine guts | `NNSpire::internal::` | Graph, training loop, file I/O — not for plugin authors |
| 3 — bundled implementations | `NNSpire::builtin::` | Dense, CpuBackend, Adam, MSE — treated same as any plugin |
...
- Phase 1: `NNSpire::layers::Dense` → `NNSpire::builtin::layers::Dense` (and all builtin types)
- Phase 2: `NNSpire::Tensor` → `NNSpire::core::Tensor` (and all Tier 1 types)
- `include/` + `src/` unified to a single shared root under `NNSpire/` (Option B layout)
