"""
NNSpire.keras.layers — Keras-compatible layer aliases.

All names match ``tf.keras.layers.*`` so that::

    from NNSpire.keras import layers
...
-----------------------
Keras ``Dense(units)`` vs NNSpire ``Linear(in, out)``.
The NNSpire ``nn.Linear`` binding accepts ``(in_features, out_features)``
...
class LSTM:
    """Placeholder — LSTM is not yet implemented in NNSpire Phase 1.
    ...
    def __init__(self, *args, **kwargs):  # noqa: ANN002, ANN003
        raise NotImplementedError(
            "NNSpire.keras.layers.LSTM is not yet implemented in Phase 1. "
            "Use NNSpire.nn.MultiheadAttention for transformer-style attention."
        )
