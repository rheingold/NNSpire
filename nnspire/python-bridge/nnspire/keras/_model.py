"""
NNSpire.keras._model — Keras-compatible Model class.

The ``Model`` class wraps any ``NNSpire.nn.Module`` (e.g. Sequential)
...
class Model:
    """Keras-compatible training wrapper for an ``NNSpire.nn.Module``.

    Parameters
    ----------
    model : NNSpire.nn.Module
        The neural network to train (e.g. an ``NNSpire.nn.Sequential``).
    ...
    ::
        ...
        import NNSpire
        from NNSpire.keras import Model
        from NNSpire.keras import layers, losses, optimizers

        seq = NNSpire.nn.Sequential()
        seq.add(NNSpire.nn.Linear(2, 4))
        seq.add(NNSpire.nn.ReLU())
        seq.add(NNSpire.nn.Linear(4, 1))

        m = Model(seq)
        m.compile(optimizer=optimizers.Adam(lr=1e-3),
                  loss=losses.MeanSquaredError())

        x = NNSpire.ones([100, 2])
        y = NNSpire.zeros([100, 1])
    ...
    def compile(
        ...
        ----------
        optimizer
            An NNSpire optimizer, e.g. ``optimizers.Adam(lr=1e-3)``.
        loss
            An NNSpire loss function, e.g. ``losses.MeanSquaredError()``.
