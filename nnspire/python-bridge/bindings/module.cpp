/* ============================================================================
 * module.cpp — pybind11 Python bindings for NNSpire
 * LGPL v3
 *
 * Exposes NNSpire-core, NNSpire-builtin to Python as the `NNSpire` package.
 *
 * NAMING CONVENTION
 * ─────────────────
 * All public names mirror PyTorch / LibTorch for maximum drop-in compatibility:
 *   NNSpire.nn.Linear(in, out)       ← matches torch.nn.Linear
 *   NNSpire.optim.Adam(lr=1e-3)      ← matches torch.optim.Adam
 *   NNSpire.nn.functional.relu(x)    ← matches torch.nn.functional.relu
 *
...
using Shape     = NNSpire::core::Shape;
using DType     = NNSpire::core::DType;
using Device    = NNSpire::core::Device;
using ILayer    = NNSpire::core::ILayer;
using Parameter = NNSpire::core::Parameter;

namespace nsl = NNSpire::builtin::layers;
namespace nsa = NNSpire::builtin::activations;
namespace nso = NNSpire::builtin::optimizers;
namespace nsloss = NNSpire::builtin::losses;

// ─── Module entry point ──────────────────────────────────────────────────────
PYBIND11_MODULE(NNSpire, m) {
    m.doc() = "NNSpire — Neural Network engine Python bindings (torch-compatible API)";
    ...
    // ── nn submodule ─────────────────────────────────────────────────────────
    auto nn = m.def_submodule("nn", "NNSpire.nn — torch-compatible layer API");
    ...
    // NNSpire Dense(outFeatures, useBias, init); in_features is informational only.
...
Matches torch.nn.Linear(in_features, out_features, bias=True).)doc");
    ...
    // NNSpire: MultiHeadAttention(numHeads, embDim, causal, dropout) — reversed!
    ...
    // NNSpire: BatchNorm1d(eps, momentum); num_features and affine not stored.
    ...
    // Python usage: model = NNSpire.nn.Sequential(); model.add(layer)
    py::class_<NNSpire::core::Sequential, ILayer>(nn, "Sequential")
        .def(py::init<>())
        .def("add", [](NNSpire::core::Sequential& s,
                       std::shared_ptr<ILayer> l) -> NNSpire::core::Sequential& {
            return s.add(l);
        }, "layer"_a, py::return_value_policy::reference_internal)
        .def("size", &NNSpire::core::Sequential::size)
        .def("layers", [](const NNSpire::core::Sequential& s) {
            return s.layers();
        })
        .def("build", [](NNSpire::core::Sequential& s, std::vector<int64_t> shape) {
            auto r = s.build(shape);
            if (!r.ok()) throw std::runtime_error(r.error().message);
            return r.value();
        })
        .def("forward", [](NNSpire::core::Sequential& s, const Tensor& x) {
            auto r = s.forward(x);
            if (!r.ok()) throw std::runtime_error(r.error().message);
            return r.value();
        });
    ...
    auto fn = nn.def_submodule("functional",
                               "NNSpire.nn.functional — stateless activation ops");
    ...
    auto optim = m.def_submodule("optim",
                                 "NNSpire.optim — torch-compatible optimizer API");
