 ...
 * shim aliases the NNSpire engine correctly.  If this file compiles and all
...
protected:
    void SetUp() override {
        if (!NNSpire::core::BackendRegistry::instance().has("cpu"))
            CpuBackend::init();
    }
};
...
TEST_F(TorchCompatTest, DtypeConstants) {
    ...
    EXPECT_EQ(t.dtype(), NNSpire::core::DType::Float32);
...
TEST_F(TorchCompatTest, Functional_ReLU) {
    auto x   = NNSpire::core::Tensor::full({3}, -1.0f);
