 ...
 *   • which are NNStudio-only extensions that will need adapters on export
 *
 * USAGE
 * ─────
 *   ComputeGraph g;
 ...
 * equivalent are marked with the custom-op domain prefix "com.nnstudio.*".
 * ============================================================================ */
 ...
 * or exclusively an NNStudio extension.
 *
 * Represented as a bitmask so a layer can be "both" standard.
 */
enum class CompatClass : uint8_t {
    NNStudioExtension = 0x00,   ///< Not found in PyTorch or Keras standard surface
...
constexpr bool isExtension     (CompatClass c) noexcept { return c == CompatClass::NNStudioExtension; }
...
struct NodeCompatInfo {
    ...
    std::string onnxOp;     ///< ONNX operator name (opset 17); "com.nnstudio.*" if extension
};
...
struct CompatReport {
    ...
    /** True if any node is an NNStudio extension (not in torch or keras). */
     ...
     *   NNStudioExtension — one or more layers are extension-only
     */
     ...
     *
     * Example:
     *   "WARNING: 2 NNStudio-extension op(s) found:\n"
     *   "  [0] type=FancyLayer   name=fancy  onnx=com.nnstudio.FancyLayer\n"
     */
...
public:
     ...
     * "BatchNormalization", "Relu").  For NNStudio extensions the string is
     * prefixed with the custom-op domain: "com.nnstudio.<typeName>".
     */
