...
Third-party plugins that are bundled with the NNStudio installer for convenience,
but that have **no special access** to engine internals beyond `core/include/nnstudio/core/`.
...
NNStudio's own built-in layers and backends live in `../builtin/` and are compiled into
`nnstudio-builtin` — a separate CMake target that links `nnstudio-core` exactly as any
