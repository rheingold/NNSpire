 ...
 *   3.  classify() — NNStudio extension (ComputeGraph, unknown)
 *   4.  onnxOp()   — standard layers map to correct ONNX op names
 *   5.  onnxOp()   — extension layers get com.nnstudio.* prefix
 ...
 *   9.  CompatReport::overallLevel() — mixed graph → NNStudioExtension
...
TEST(CompatibilityCheckerTest, OnnxOp_UnknownLayer_CustomDomain)
{
    ...
    EXPECT_EQ(op, "com.nnstudio.FancyLayer");
}
...
/// Minimal ILayer that reports itself as an unknown NNStudio-extension layer.
...
TEST(CompatibilityCheckerTest, Check_GraphWithExtension_HasExtensions)
{
    ...
    EXPECT_EQ(r.overallLevel(), CompatClass::NNStudioExtension);
...
TEST(CompatibilityCheckerTest, ExtensionNodes_OnlyExtensionsReturned)
{
    ...
    EXPECT_EQ(exts[0].onnxOp,   "com.nnstudio.FancyLayer");
}
