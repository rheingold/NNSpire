# NNStudio C++ Layer Plugin Scaffold

This scaffold generates a C++ shared library that implements a custom
NNStudio layer plugin.

## Prerequisites

- NNStudio SDK (source tree or installed headers)
...
2. **Generate signing key + CSR**:
   ...
   ```sh
   nnstudio-sign keygen --type layer --id com.example.my-layer --out .
   ```

3. **Build the plugin**:

   ```sh
   cmake -B build -DNNSTUDIO_SDK_DIR=/path/to/nnstudio/plugin-api
   cmake --build build
   ```
...
6. **Install the plugin** — copy the `.dll`/`.so`, `plugin.manifest.json`,
   and `.p7s` to one of the NNStudio plugin directories:
...
```sh
nnstudio-sign verify --manifest plugin.manifest.json --behavioral-check
```
