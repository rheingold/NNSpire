"""
nnstudio.torch_compat — drop-in replacement for ``import torch``.

Usage::

    import nnstudio.torch_compat as torch   # instead of: import torch
    import nnstudio.torch_compat.nn as nn   # instead of: import torch.nn as nn
    import nnstudio.torch_compat.nn.functional as F

Any code that uses only the standard PyTorch API surface should work
unchanged after swapping the import.  NNStudio-extension features that go
beyond the standard PyTorch surface remain accessible via ``import nnstudio``
directly.

Covered aliases
---------------
* ``torch.Tensor`` → ``nnstudio.Tensor``
* ``torch.zeros / ones / full`` → nnstudio factories
* ``torch.float32 / float16 / int8 / int32 / bool`` → ``nnstudio.DType.*``
* ``torch.nn`` → ``nnstudio.nn``  (Linear, Conv2d, ReLU, Sequential, …)
* ``torch.optim`` → ``nnstudio.optim``  (SGD, Adam, AdamW, RMSprop)
...
import nnstudio as _ns  # always resolves to the already-loaded package
...
# ------------------------------------------------------------------
# Submodules — nnstudio.nn / optim already carry torch-compatible names.
