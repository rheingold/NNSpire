...
using namespace nnstudio::trust;
...
Result<LoadedPlugin> PluginLoader::load(std::string_view path) {
    ...
    auto fn = reinterpret_cast<DescFn>(platformSym(handle, "nnstudio_plugin_descriptor"));
    if (!fn) {
        platformClose(handle);
        return Result<LoadedPlugin>{"PluginLoader: nnstudio_plugin_descriptor symbol not found"};
    }
