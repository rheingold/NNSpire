# NNStudio — Third-party dependencies
...
curl -L https://gitlab.com/libeigen/eigen/-/archive/3.4.0/eigen-3.4.0.tar.gz \
     | tar xz --strip-components=1 -C nnstudio/third-party/eigen/
```

## Add pybind11 (header-only, MIT)
```bash
git submodule add https://github.com/pybind/pybind11.git nnstudio/third-party/pybind11
git submodule update --init
```

## Add GoogleTest (BSD 3-Clause, test builds only)
```bash
git submodule add https://github.com/google/googletest.git nnstudio/third-party/googletest
git submodule update --init
```
...
# vcpkg install onnx
# Or: git submodule add https://github.com/onnx/onnx.git nnstudio/third-party/onnx
```
