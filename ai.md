# ai.md — NNStudio AI Assistant Knowledge Base

This file is the persistent memory for AI coding assistants working on NNStudio.
...
- **Name**: NNStudio (full), Studio (short internal reference)
...
### Language stack
- **Core engine** (`nnstudio/core/`, `nnstudio/backends/`, `nnstudio/plugin-api/`): C++17
- **UI**: Qt 6 + QML (primary); Qt 5.15 LTS secondary build config for older Linux/macOS
- **Python plugins & bridge**: pybind11 exposing `nnstudio-core` to Python
...
- Top-level `CMakeLists.txt` in `nnstudio/`
- Build directory: `build/` (gitignored)
- Target names: `nnstudio-core` (static lib), `nnstudio-plugin-api` (interface lib/headers), `NNStudio` (executable), `nnstudio-runner` (optional sidecar)

### Qt version policy
- Primary target: Qt 6.5 LTS or Qt 6.7+
- All Qt 6-only APIs isolated behind `nnstudio/app/qt_version_helpers.h`
...
- All public API headers are in `include/nnstudio/` subdirs — no implementation in public headers except templates
...
| Path | What it is |
|---|---|
| `nnstudio/core/tensor/` | `Tensor<T>` — shape, strides, dtype, device, ops |
| `nnstudio/core/layers/` | Abstract `Layer` + built-ins (Dense, Conv2D, MHA, etc.) |
| `nnstudio/core/activations/` | Activation functions with forward value AND derivative |
| `nnstudio/core/losses/` | Loss functions (MSE, CrossEntropy, Huber, BCE) |
| `nnstudio/core/optimizers/` | SGD, Adam, AdamW, RMSProp with LR schedulers |
| `nnstudio/core/graph/` | `ComputeGraph` — DAG of ops, autograd, drives visual editor |
| `nnstudio/core/training/` | `Trainer` — training loop, callbacks, checkpoint save/load |
| `nnstudio/core/formats/` | `OnnxIO`, `NNSFormat` (`.nns` project file) |
| `nnstudio/core/features/` | `FeatureFlags.h` — all flags start `FREE` |
| `nnstudio/backends/` | `CpuBackend`, `CudaBackend`, `QuantumBackend` (stub), `BackendRegistry` |
| `nnstudio/plugin-api/` | `nnstudio_plugin.h` (C ABI), `TrustVerifier`, `TrustUpdateHandler`, `PluginLoader` |
| `nnstudio/plugin-api/trust/` | `root_ca.pem` (embedded seed), TrustStore management, TUP schema |
| `docs/adr/` | Architecture Decision Records — ADR-001 to ADR-019; index at `docs/adr/README.md` |
| `nnstudio/plugins/` | Reference plugins: BPE tokenizer, FAISS index, example custom activation |
| `nnstudio/pipeline/` | Input adapters, tokenization chain, context stage, execution, output adapters |
| `nnstudio/deployment/` | Export wizards, runner clients (Triton/TFS/KServe/ORT), bundle builder |
| `nnstudio/python-bridge/` | pybind11 module; `runners/` sub-package mirrors deployment connectors |
| `nnstudio/app/` | Qt 6 QML application — `main.cpp`, controllers, QML components |
| `nnstudio/tests/` | GoogleTest unit + integration tests |
...
All Studio features are registered in `nnstudio/core/features/FeatureFlags.h`.
...
- [ ] C++ class/function in `nnstudio/core/` or `nnstudio/plugin-api/` or `nnstudio/backends/`
- [ ] Python binding in `nnstudio/python-bridge/`
- [ ] C++ plugin scaffold template in `nnstudio/plugin-api/templates/cpp/`
- [ ] Python plugin scaffold template in `nnstudio/plugin-api/templates/python/`
...
- **`nnstudio-runner` sidecar** — optional, crash-isolated inference process for untrusted plugins. Off by default.
