# NNStudio — Plugin SDK Whitepaper
...
## 3. C ABI header (`nnstudio_plugin.h`)

```c
/* nnstudio_plugin.h  —  LGPL v3  —  PUBLIC API, C linkage only */
...
Additional vtable structs for `NN_PLUGIN_ACTIVATION`, `NN_PLUGIN_OPTIMIZER`, `NN_PLUGIN_BACKEND`, `NN_PLUGIN_INPUT_ADAPTER`, `NN_PLUGIN_OUTPUT_ADAPTER`, `NN_PLUGIN_CONTEXT_SOURCE`, and `NN_PLUGIN_RUNNER_CLIENT` follow the same pattern. Full definitions in `nnstudio/plugin-api/nnstudio_plugin.h`.
...
The detached PKCS#7 signature (`signature` field) covers the UTF-8 canonical JSON of this manifest plus all binary artefacts listed in `implementations`. Generated by `nnstudio-sign sign`.
...
2. `<OS system plugin dir>/nnstudio/` — system-wide installed plugins
3. `<user home>/.nnstudio/plugins/` — user-installed plugins
...
The scaffold is generated by `nnstudio-sign keygen --type layer --id com.example.my-layer --out my-layer/`.
...
```cpp
#pragma once
#include <nnstudio/plugin-api/nnstudio_plugin.h>
...
static const NNPluginDescriptor DESCRIPTOR = {
    NNSTUDIO_PLUGIN_API_VERSION, NN_PLUGIN_LAYER,
    "com.example.my-layer", "My Layer", "1.0.0", "Example Corp", "MIT",
    create, destroy, &VTABLE
};

extern "C" const NNPluginDescriptor* nnstudio_plugin_descriptor() {
    return &DESCRIPTOR;
}
```
...
Generated by `nnstudio-sign keygen --type layer --id com.example.my-layer --lang python --out my-layer-py/`.
...
```python
# my_layer/layer.py
from nnstudio import Tensor, LayerPlugin, plugin_descriptor
...
```
nnstudio-sign <subcommand> [options]
...
Subcommands:
  ...
  submit        Submit CSR to NNStudio Plugin Registry
                --csr plugin.csr
                --registry https://registry.nnstudio.dev
  ...
  create-tup    Build and sign a Trust Update Package
                ...
                --out trust-update.nnstudio-trust
...
- Behavioural equivalence test: a set of reference input tensors produces tensors within float32 epsilon between C++ and Python implementations (automated by `nnstudio-sign verify --behavioral-check`)
