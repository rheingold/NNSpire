...
The early code had `core/include/nnstudio/` containing headers that declared
`namespace nnstudio { ... }`. The `include/nnstudio/` prefix disappeared at the
include path boundary but was not reflected in the C++ namespace, creating a
mismatch: `#include <nnstudio/Tensor.h>` implied `nnstudio::Tensor`, but nothing
...
include paths were scoped to each target. Option B used a single `nnstudio/include/`
root for all targets. The tie-breaking argument: `nnstudio-builtin` has a permanent,
legitimate dependency on `nnstudio-core` (builtin layers inherit from `ILayer`, etc.).
...
If the plugin's namespace were derived from its position under `nnstudio/plugins/`,
...
```
nnstudio/include/core/Tensor.h        →   nnstudio::core::Tensor
nnstudio/include/builtin/layers/Dense.h  →   nnstudio::builtin::layers::Dense
nnstudio/src/core/Tensor.cpp          →   nnstudio::core:: (same namespace, build-only)
```
...
|---|---|---|---|
| 1 — public contract | `nnstudio::core::` | Abstract interfaces (`ILayer`, `IBackend`, `ILoss`, `IOptimizer`), data types (`Tensor`, `Result<T>`, `Parameter`), engine services (`Trainer`, `BackendRegistry`) | Versioned; breaking changes require migration period |
| 2 — engine internals | `nnstudio::internal::` | Autograd graph, training loop, format I/O — not for plugin authors | May change between minor releases |
| 2b — UI extension | `nnstudio::ui::` | QML panel registration, property/signal contracts | Evolves with Qt version |
| 3 — bundled implementations | `nnstudio::builtin::` | NNStudio's own layers, backends, losses, optimizers — treated identically to third-party plugins | No special access over any plugin |

No concrete implementation lives in `nnstudio::core::`. Tier 3 (`builtin`) uses
`using namespace nnstudio::core;` inside its namespace blocks so it can reference Tier 1
types without full qualification; this does not add any include-path coupling beyond
what `target_link_libraries(nnstudio-builtin PUBLIC nnstudio-core)` already expresses.
...
nnstudio/
    ...
    tests/
    CMakeLists.txt     ← all nnstudio-core / nnstudio-builtin targets defined here
```
...
- The `plugin-api/include/nnstudio/` subtree is a deliberate exception: it uses the
  pre-existing `<nnstudio/nnstudio_plugin.h>` include path for the C ABI header because
  that path is already in external documentation and plugin templates. It does not
  declare a `namespace nnstudio` — it is pure C, so the path=namespace rule does not
  apply.
...
- `src/internal/` subtree (graph, training, formats) will follow the same rule when
  implemented: `src/internal/graph/` → `nnstudio::internal::graph::`.
